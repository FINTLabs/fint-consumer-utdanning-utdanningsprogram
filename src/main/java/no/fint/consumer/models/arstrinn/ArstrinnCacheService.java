...
@Slf4j
@Service
public class ArstrinnCacheService extends CacheService<FintResource<Arstrinn>> {
    ...
    public ArstrinnCacheService() {
        super(MODEL, UtdanningsprogramActions.GET_ALL_ARSTRINN);
    }
    ...
    public Optional<FintResource<Arstrinn>> getArstrinnBySystemId(String orgId, String systemId) {
        return getOne(orgId, (fintResource) -> Optional
                .ofNullable(fintResource)
                .map(FintResource::getResource)
                .map(Arstrinn::getSystemId)
    ...
    public void onAction(Event event) {
        update(event, new TypeReference<List<FintResource<Arstrinn>>>() {
        });
    }
}

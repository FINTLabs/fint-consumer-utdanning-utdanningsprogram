...
public class SkoleController {
    ...
    public SkoleResource getSkoleBySkolenummer(
        ...
        if (cacheService != null) {
            ...
            SkoleResource skole = objectMapper.convertValue(response.getData().get(0), SkoleResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.toResource(skole);
        }    
    }
    ...
    public ResponseEntity getStatus(
        ...
        switch (event.getResponseStatus()) {
            case ACCEPTED:
                ...
                result = objectMapper.convertValue(event.getData(), objectMapper.getTypeFactory().constructCollectionType(List.class, SkoleResource.class));
    ...
    @PostMapping
    public ResponseEntity postSkole(
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody SkoleResource body,
            @RequestParam(name = "validate", required = false) boolean validate
    ) {
        log.debug("postSkole, Validate: {}, OrgId: {}, Client: {}", validate, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, UtdanningsprogramActions.UPDATE_SKOLE, client);
    ...
    @PutMapping("/skolenummer/{id:.+}")
    public ResponseEntity putSkoleBySkolenummer(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody SkoleResource body
    ) {
        log.debug("putSkoleBySkolenummer {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, UtdanningsprogramActions.UPDATE_SKOLE, client);
        event.setQuery("skolenummer/" + id);
    ...
    public ResponseEntity putSkoleBySystemId(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody SkoleResource body
    ) {
        log.debug("putSkoleBySystemId {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, UtdanningsprogramActions.UPDATE_SKOLE, client);
        event.setQuery("systemid/" + id);
    ...
    public ResponseEntity putSkoleByOrganisasjonsnummer(
            @PathVariable String id,
            @RequestHeader(name = HeaderConstants.ORG_ID) String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT) String client,
            @RequestBody SkoleResource body
    ) {
        log.debug("putSkoleByOrganisasjonsnummer {}, OrgId: {}, Client: {}", id, orgId, client);
        log.trace("Body: {}", body);
        linker.mapLinks(body);
        Event event = new Event(orgId, Constants.COMPONENT, UtdanningsprogramActions.UPDATE_SKOLE, client);
        event.setQuery("organisasjonsnummer/" + id);

package no.novari.fint.consumer.models.utdanningsprogram;
...
@Slf4j
@Api(tags = {"Utdanningsprogram"})
@CrossOrigin
@RestController
@RequestMapping(name = "Utdanningsprogram", value = RestEndpoints.UTDANNINGSPROGRAM, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class UtdanningsprogramController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Utdanningsprogram cache is disabled.");
        }
    ...
    @GetMapping
    public UtdanningsprogramResources getUtdanningsprogram(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getUtdanningsprogramByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Utdanningsprogram cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<UtdanningsprogramResource> resources;
    ...
    @PostMapping("/$query")
    public UtdanningsprogramResources getUtdanningsprogramByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getUtdanningsprogram(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private UtdanningsprogramResources getUtdanningsprogramByOdataFilter(
        ...
        Event event = new Event(
                ...
                UtdanningsprogramActions.GET_UTDANNINGSPROGRAM, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new UtdanningsprogramResources();
    
        ArrayList<UtdanningsprogramResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<UtdanningsprogramResource>>() {});
    ...
    public UtdanningsprogramResource getUtdanningsprogramBySystemId(
        ...
        if (cacheService != null) {
            ...
            Optional<UtdanningsprogramResource> utdanningsprogram = cacheService.getUtdanningsprogramBySystemId(orgId, id);

            fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

            return utdanningsprogram.map(linker::toResource).orElseThrow(() -> new EntityNotFoundException(id));
            ...
            UtdanningsprogramResource utdanningsprogram = objectMapper.convertValue(response.getData().get(0), UtdanningsprogramResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(utdanningsprogram);
        }    
    }

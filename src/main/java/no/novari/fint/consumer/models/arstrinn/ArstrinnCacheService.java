...
@Slf4j
@Service
@ConditionalOnProperty(name = "fint.consumer.cache.disabled.arstrinn", havingValue = "false", matchIfMissing = true)
public class ArstrinnCacheService extends CacheService<ArstrinnResource> {
    ...
    @Scheduled(initialDelayString = Constants.CACHE_INITIALDELAY_ARSTRINN, fixedRateString = Constants.CACHE_FIXEDRATE_ARSTRINN)
    ...
    public void onAction(Event event) {
        ...
        if (UtdanningsprogramActions.valueOf(event.getAction()) == UtdanningsprogramActions.UPDATE_ARSTRINN) {
            if (event.getResponseStatus() == ResponseStatus.ACCEPTED || event.getResponseStatus() == ResponseStatus.CONFLICT) {
                List<CacheObject<ArstrinnResource>> cacheObjects = data
                    .stream()

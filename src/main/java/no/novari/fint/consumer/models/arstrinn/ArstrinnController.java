package no.novari.fint.consumer.models.arstrinn;
...
public class ArstrinnController {
    ...
    public Map<String, String> getLastUpdated(@RequestHeader(name = HeaderConstants.ORG_ID, required = false) String orgId) {
        if (cacheService == null) {
            throw new CacheDisabledException("Arstrinn cache is disabled.");
        }
    ...
    public ArstrinnResources getArstrinn(
        ...
        if (cacheService == null) {
            if (StringUtils.isNotBlank($filter)) {
                return getArstrinnByOdataFilter(client, orgId, $filter);
            }
            throw new CacheDisabledException("Arstrinn cache is disabled.");
        }
        if (props.isOverrideOrgId() || orgId == null) {
        ...
        Stream<ArstrinnResource> resources;
    ...
    @PostMapping("/$query")
    public ArstrinnResources getArstrinnByQuery(
            @RequestHeader(name = HeaderConstants.ORG_ID, required = false)   String orgId,
            @RequestHeader(name = HeaderConstants.CLIENT, required = false) String client,
            @RequestParam(defaultValue = "0") long sinceTimeStamp,
            @RequestParam(defaultValue = "0") int  size,
        ...
        return getArstrinn(orgId, client, sinceTimeStamp, size, offset, query, request);
    }

    private ArstrinnResources getArstrinnByOdataFilter(
        ...
        Event event = new Event(
                ...
                UtdanningsprogramActions.GET_ARSTRINN, client);
        event.setOperation(Operation.READ);
        ...
        if (response.getData() == null || response.getData().isEmpty())
            return new ArstrinnResources();
    
        ArrayList<ArstrinnResource> list = objectMapper.convertValue(
                response.getData(),
                new TypeReference<ArrayList<ArstrinnResource>>() {});
    ...
    public ArstrinnResource getArstrinnBySystemId(
        ...
        if (cacheService != null) {
            ...
            ArstrinnResource arstrinn = objectMapper.convertValue(response.getData().get(0), ArstrinnResource.class);

            fintAuditService.audit(response, Status.SENT_TO_CLIENT);

            return linker.mapAndResetLinks(arstrinn);
        }    
    }
